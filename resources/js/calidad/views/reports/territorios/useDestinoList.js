...
export default function useDestinoList(pais){
  
  const tableColumns = computed(() => {
    if(pais.value){
      return [
          {key:'id',label:'#',sortable:false},
          {key:'nombre',label:'Destino',sortable:true},
          {key:'pais',label:'Pais',sortable:true},
      ]
    }
    return [
      {key:'pais',label:'Pais',sortable:true},
      {key:'destinos',label:'Cant Destinos',sortable:true},
    ]
  })
  ...
  const fetchData = (ctx,n) => {
    ...
    store.dispatch('destino/reportFetchDestino',{
      ...
      mes:mes.value,
      pais:pais.value
    }).then(({total:all,destinos,totalPaises:tp}) => {
      total.value = all
  ...
  const descargar = () =>{
    store.dispatch('destino/reportFetchDestinoDescargar',{
      page:currentPage.value,
      perPage:perPage.value,
      sortBy:sortBy.value,
      isSortDirDesc:isSortDirDesc.value,
