...
import useEquipoList from './useEquipoList.js'
...
export default {
  ...
  setup(props) {
    ...
    const actions = useEquipoList(pais);
    ...
    const fileSelected = (item) => {
      if (item.length) {
          router.push({ path: '/reports/equipo', query: { pais: item[0].pais ?? 'otros' } })
      } else {
        router.push({ path: '/reports/equipo' })
      }
    }
