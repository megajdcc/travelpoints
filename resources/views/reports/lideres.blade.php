...
<head>
	...
	<title>Lideres y sus resultados</title>
		...
		<div class="container-header">
			...
			<table class="table-1" border="0">
				<tbody>
					<tr>
						<td >
							<h2>Líderes y sus resultados</h2>
						</td>
			...
			<table class="table-contenido"  border="0">
				<thead class="bg-danger">
					<tr>
						<th class="">No.</th>
						<th class="">Lider</th>
						<th class="">Promotores Activos</th>
						<th class="">Estatus</th>
						<th class="">Comisiones ganadas</th>
					</tr>
				...
				<tbody>
					...
					@foreach ($lideres as $key => $lider)
						<tr style="border-bottom:1px solid rgba(0,0,0,0.4)">
								<td>{{ $key + 1  }}</td>
                <td>{{ $lider->getNombreCompleto() }}</td>
								<td>{{ $lider->promotores->count() }}</td>
								<td>{{ $lider->getStatus() }}</td>
                <td class="border-success">{{  $lider->comision }}</td>
							</tr>
					@endforeach
				</tbody>
			</table>
