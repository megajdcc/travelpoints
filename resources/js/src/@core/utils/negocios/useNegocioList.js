...
export default function useNegocioList({destino = null , atraccion = null}) {
  ...
  const refetchData = () => fetchData((negocios) => items.value = negocios)
  ...
  const fetchData = (next) => {
    ...
    store.dispatch('negocio/fetchDataPublic', {
