...
<body>
	...
	<div class="container">
					...
					<tr>
						<td colspan="2">
							@if(isset($filtro['pais']) && !empty($filtro['pais']))
								<h2>Gasto turístico por destino: <b>{{ $filtro['pais'] }}</b></h2>
							@else
								<h2>Gasto turístico por Países</h2>
							@endif
						</td>
					</tr>
		...
		<div class="container-cliente">
			<table class="table-contenido"  border="0">
				<thead>
					<tr>
						@if(isset($filtro['pais']) && !empty($filtro['pais']))
								...
								<th class="">Total operaciones</th>
								<th class="">Reg Promedio</th>
						@else
								<th class="">Num.</th>
								<th class="">País</th>
								<th class="">Total compras</th>
				...
				<tbody>
                    ...
                    <td>{{ $registro->total_compras }}</td>
										<td>{{ $registro->total_operaciones }}</td>
										<td>{{ $registro->reg_promedio }}</td>
								@else
										<td>{{ $registro->pais }}</td>
										<td>{{ $registro->total_compras }}</td>
										<td>{{ $registro->total_operaciones }}</td>
      ...
      <table class="table-1" border="0">
        <tr>
          <td class="text-left"><h4>Cinco países con mayores registros</h4></td>
        </tr>
      </table>
       ...
       <table border="0" class="table-1" >
            ...
            <tbody>
              @foreach ($paises_con_mayor_registros as $r)
                  <tr>
                    <td>{{ $r->pais }}</td>
                    <td>{{ $r->ventas }}</td>
                    <td>{{ round($r->promedio) }}%</td>
                  </tr>
              @endforeach
            </tbody>
        </table>
