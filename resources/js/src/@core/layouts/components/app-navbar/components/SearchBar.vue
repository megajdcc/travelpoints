<template>
  <li class="nav-item nav-search">
    ...
    <div class="search-input" :class="{'open': showSearchBar}">
      ...
      <b-form-input v-if="showSearchBar" v-model="searchQuery" placeholder="Busque en Boda y Playa" autofocus
        autocomplete="off" @keyup.up="increaseIndex(false)" @keyup.down="increaseIndex"
      ...
      <li v-for="(suggestion_list, grp_name, grp_index) in filteredData" :key="grp_index" class="suggestions-groups-list">
        ...
        <ul>
          <li v-for="(suggestion, index) in suggestion_list" :key="index" class="suggestion-group-suggestion cursor-pointer"
            ...
            <b-link v-if="grp_name === 'paginas'" class="p-0">
            ...
            <b-link v-else-if="grp_name === 'bodas' && $can('write','bodas')" class="p-0" >
              <feather-icon :icon="suggestion.icon" class="mr-75" />
              <span class="align-middle">{{ suggestion.title }}</span>
            </b-link>

            <!-- <b-link v-else-if="grp_name === 'reservaciones' && $can('update','reservaciones')" class="p-0" >
              <feather-icon :icon="suggestion.icon" class="mr-75" />
              <span class="align-middle">{{ suggestion.title }}</span>
            </b-link> -->
...
export default {
  ...
  setup() {
    ...
    const suggestionSelected = (grpName, suggestion) => {
      ...
      if (grpName === 'paginas' || grpName === 'bodas') router.push(suggestion.route).catch(() => {})
    ...
    const {
      ...
      resetsearchQuery,
      filteredData,
    } = useAutoSuggest({ data: {paginas:paginas.value, bodas:bodas.value}, searchLimit: 4 })
